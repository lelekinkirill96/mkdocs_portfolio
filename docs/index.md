# Проблема "слепых зон": нельзя защитить то, чего не видишь

## Что такое "слепые зоны" в IT-инфраструктуре?
Это устройства, сервисы или сегменты сети, которые не учтены в системах мониторинга, CMDB или Active Directory. Они возникают по следующим причинам:

- **Теневое IT** — устройства, подключённые без ведома ИТ-службы:
  - Тестовые серверы разработчиков.
  - Личные ноутбуки сотрудников.
  - IoT-устройства (IP-камеры, умные розетки, датчики).

- **Новые устройства** — серверы, сетевое оборудование или виртуальные машины, добавленные в обход процессов ИТ-отдела.

- **Изолированные сегменты** — DMZ, промышленные сети (OT), облачные инфраструктуры, которые не сканируются регулярно.

- **Устаревшие записи** — устройства, давно выведенные из эксплуатации, но всё ещё числящиеся в документации.
...
**Решение:** Регулярное сканирование сети (Discovery) — первый шаг к полному контролю над инфраструктурой.

---

## 2. Первичная инвентаризация: создание эталонного слепка (baseline) сети
...
✅ Контроль изменений — отслеживание появления/исчезновения устройств.
...
## 3. Механика Discovery: методы и настройки
...
## Как Vulns.io определяет ОС, имена хостов и тип оборудования
...
## Импорт активов в базу для анализа уязвимостей
...
## 5. Автоматизация: настройка расписания и контроль изменений
...
## 6. Резюме: Discovery как основа управления уязвимостями
1. **Discovery** — это не разовое действие, а непрерывный процесс.
   - Регулярное сканирование позволяет обнаруживать новые устройства и отслеживать изменения.

2. На выходе получаем не просто список IP, а **структурированную базу данных активов**:
   - Каждое устройство классифицировано по критичности.
   - Данные об активах обогащены информацией об ОС, ПО, открытых портах.

3. **Инвентаризация** — это первый шаг к полноценному управлению уязвимостями:
